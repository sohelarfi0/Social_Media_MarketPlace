  ...
  return (
    <div className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-100 flex items-center justify-center sm:p-4'>
        <div className='bg-white sm:rounded-lg shadow-2xl w-full max-w-lg h-screen sm:h-[320px] flex flex-col'>
            {/* Header */}
            <div className='bg-gradient-to-r from-indigo-600 to bg-indigo-400 text-white p-4 sm:rounded-t-lg flex items-center justify-between'>
                <div className='flex-1 min-w-0'>
                    <h3>{listing?.title}</h3>
                    <p className=''>Adding Credentials for {listing?.username} on {listing?.platform}</p>
                </div>
            ...
            <form onSubmit={handleSubmission} className='flex flex-col items-start gap-4 p-4 overflow-y-scroll'>
                {credential.map((cred, index)=>(
                    <div key={cred.type} className='grid grid-cols-[2fr_3fr_1fr] items-center gap-2'>
                        ...
                        <input type="text" vaalue={cred.value} onChange={(e)=>setCredential((prev)=>prev.map((c,i)=>(i===index ? {...c, value:e.target.value}: c)))} 
                        className='w-full px-2 py-1.5 text-sm border border-gray-300 roundeed outline-indigo-400' />
                ...
                <div className='flex items-center gap-2'>
                    ...
                    <button type='button' onClick={handleAddField} className='flex items-centerr gap-2 text-sm text-gray-600 hover:text-gray-700 cursor-pointer'>
                        <CirclePlus className='w-5 h-5'/>
                    </button>
                </div>
                {/* Submit Button */}
                <button type='submit' className='bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 mt-4 round-md'>Submit</button>
            </form>
        </div>
