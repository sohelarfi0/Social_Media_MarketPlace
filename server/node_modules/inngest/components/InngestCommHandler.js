import { ExecutionVersion, defaultMaxRetries, envKeys, forwardedHeaders, headerKeys, internalEvents, logPrefix, probe, queryKeys, syncKind } from "../helpers/consts.js";
import { enumFromValue } from "../helpers/enum.js";
import { version } from "../version.js";
import { allProcessEnv, checkModeConfiguration, getPlatformName, inngestHeaders, parseAsBoolean } from "../helpers/env.js";
import { rethrowError, serializeError } from "../helpers/errors.js";
import { AsyncResponseType, DefaultMaxRuntime, StepMode, StepOpCode, functionConfigSchema, inBandSyncRequestBodySchema } from "../types.js";
...
import { buildWrapRequestChain } from "./middleware/utils.js";
import { internalLoggerSymbol } from "./Inngest.js";
import { _internals } from "./execution/engine.js";
	...
	*/
	...
	constructor(options) {
		...
		*/
		...
		this.streaming = z.boolean().default(defaultStreamingOption).catch((ctx) => {
	...
	*/
			...
			if (method === "POST" || forceExecution) {
				...
				*/
				...
				try {
					...
					this.client[internalLoggerSymbol].error({ err }, "Error handling execution result");
					throw err;
				}
			}
	...
	runStep({ actions, functionId, stepId, data, timer, reqArgs, headers, fn, forceExecution, headerReqVersion, requestInfo, mwInstances }) {
		...
		const result = runAsPromise(async () => {
			...
			const checkpointingConfig = fn.fn["shouldAsyncCheckpoint"](requestedRunStep, ctx?.fn_id, Boolean(ctx?.disable_immediate_execution), this.defaultMaxRuntime);
	...
	*/
	...
	async register(url, deployId, getHeaders) {
		...
		try {
			...
			this.client[internalLoggerSymbol].warn({ err }, "Couldn't unpack register response");
		...
		try {
			...
			this.client[internalLoggerSymbol].warn({ err }, "Invalid register response schema");
			let message = "Failed to register";
			if (err instanceof Error) message += `; ${err.message}`;
			message += `; status code: ${res.status}`;
			return {
