...
import { getEnvironmentName } from "../../helpers/env.js";
import { DefaultMaxRuntime } from "../../types.js";
...
function prepareConnectionConfig(apps, inngest) {
	...
	const hashedSigningKey = inngest.signingKey ? hashSigningKey(inngest.signingKey) : void 0;
	...
	const hashedFallbackKey = inngest.signingKeyFallback ? hashSigningKey(inngest.signingKeyFallback) : void 0;
	const capabilities = {
		trust_probe: "v1",
		connect: "v1"
	};
	...
	inngest[internalLoggerSymbol].debug({ functionSlugs: Object.entries(functionConfigs).map(([appId, { functions: fns }]) => {
	...
	for (const [appId, { client, functions: fns }] of Object.entries(functions)) {
		const inngestCommHandler = new InngestCommHandler({
			...
			handler: (msg) => {
				...
				const parsed = parseFnData(JSON.parse(asString), void 0, inngest[internalLoggerSymbol]);
...
//#endregion
export { prepareConnectionConfig };
