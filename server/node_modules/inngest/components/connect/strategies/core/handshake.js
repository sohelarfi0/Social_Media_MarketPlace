...
import { ConnectMessage, GatewayConnectionReadyData, GatewayMessageType, WorkerConnectRequestData, WorkerDisconnectReason, gatewayMessageTypeToJSON, workerDisconnectReasonToJSON } from "../../../../proto/src/components/connect/protobuf/connect.js";
import { AuthError, ConnectionLimitError, ReconnectError } from "../../util.js";
...
import { createStartRequest, parseConnectMessage } from "../../messages.js";
import ms from "ms";
...
async function sendStartRequest(config, hashedSigningKey, attempt, excludeGateways, logger) {
	...
	if (config.envName) headers[headerKeys.Environment] = config.envName;
	const targetUrl = new URL("/v0/connect/start", await resolveApiBaseUrl({
	...
	if (!resp.ok) {
		if (resp.status === 401) throw new AuthError(`Failed initial API handshake request to ${targetUrl.toString()}${config.envName ? ` (env: ${config.envName})` : ""}, ${await resp.text()}`, attempt);
	...
	const { parseStartResponse } = await import("../../messages.js");
...
async function establishConnection(config, hashedSigningKey, attempt, excludeGateways, logger) {
	...
	const connectTimeout = setTimeout(() => {
		...
		rejectWsConnected?.(new ReconnectError(`Connection ${connectionId} timed out`, attempt));
	}, 1e4);
	...
	const rejectHandshake = (error) => {
		...
		rejectWsConnected?.(new ReconnectError(`Error while connecting (${connectionId}): ${error instanceof Error ? error.message : "Unknown error"}`, attempt));
	};
	ws.onerror = (err) => rejectHandshake(err);
	ws.onclose = (ev) => {
		rejectHandshake(new ReconnectError(`Connection ${connectionId} closed: ${ev.reason}`, attempt));
	};
	const setupState = {
	...
	ws.onmessage = async (event) => {
		...
		if (!setupState.receivedGatewayHello) {
			if (connectMessage.kind !== GatewayMessageType.GATEWAY_HELLO) {
				rejectHandshake(new ReconnectError(`Expected hello message, got ${gatewayMessageTypeToJSON(connectMessage.kind)}`, attempt));
				return;
			}
		...
		if (!setupState.receivedConnectionReady) {
			if (connectMessage.kind !== GatewayMessageType.GATEWAY_CONNECTION_READY) {
				rejectHandshake(new ReconnectError(`Expected ready message, got ${gatewayMessageTypeToJSON(connectMessage.kind)}`, attempt));
				return;
			}
			...
			statusIntervalMs = readyPayload.statusInterval.length > 0 ? ms(readyPayload.statusInterval) : 0;
			resolveWsConnected?.();
			return;
		}
		logger.warn({
