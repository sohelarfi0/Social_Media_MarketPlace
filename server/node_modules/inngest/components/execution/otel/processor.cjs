...
var InngestSpanProcessor = class InngestSpanProcessor {
	...
	*/
	declareStepExecution(rootSpanId, id, index, hashedStepId, attempt) {
		...
		if (!isRoot) {
			if ((span.parentSpanContext?.spanId ?? span.parentSpanId) === parentState.rootSpanId) {
				...
				if (stepCtx) {
					...
					trackDebug("setting inngest.step.parentSpanId=%s (seed=%s) on span %s step %s index %d attempt %d", newSpanId, seed, spanId, stepCtx.id, stepCtx.index, stepCtx.attempt);
					span.setAttribute(require_consts.Attribute.InngestStepParentSpanId, newSpanId);
					span.setAttribute(require_consts.Attribute.InngestStepId, stepCtx.id);
					span.setAttribute(require_consts.Attribute.InngestStepIndex, stepCtx.index);
					span.setAttribute(require_consts.Attribute.InngestStepHash, stepCtx.hashedStepId);
					span.setAttribute(require_consts.Attribute.InngestStepAttempt, stepCtx.attempt);
				}
			}
		}
